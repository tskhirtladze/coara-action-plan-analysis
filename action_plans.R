...
# install.packages(c("pdftools", "stringr", "dplyr", "purrr", "tools"))
...
terms <- list(
  ...
  "helsinki initiative" = c("helsinki initiative", "helsinki initiative on multilingualism in scholarly communication"),
  ...
  "data management plan" = c("dmp","data management plan","research data management plan"),
  ...
  "barcelona declaration" = c("barcelona declaration", "barcelona declaration on open research information"),
  ...
  "open science officers" = c("open science officer","open-science officers","open science officers"),  # Fixed typo: "open-sciece" -> "open-science"
  "open science awareness" = c("open science awareness")
)

# -----------------------------
# OVERLAP-AWARE TERM COUNTING
# -----------------------------

count_terms_no_overlap <- function(text, terms) {
  term_table <- purrr::imap_dfr(terms, function(synonyms, term_name) {
    data.frame(term = term_name, surface = synonyms, stringsAsFactors = FALSE)
  }) %>%
    mutate(length = nchar(surface)) %>%
    arrange(desc(length))
  
  claimed <- rep(FALSE, nchar(text))
  
  counts <- setNames(rep(0, length(terms)), names(terms))
  
  for(i in seq_len(nrow(term_table))) {
    
    surface    <- term_table$surface[i]
    term_name  <- term_table$term[i]
    
    pattern <- paste0("\\b", str_replace_all(surface, "([\\W])", "\\\\\\1"), "\\b")
    ...
    for(j in seq_len(nrow(matches))) {
      ...
      if(any(claimed[start:end])) next
      
      claimed[start:end] <- TRUE
      counts[term_name] <- counts[term_name] + 1
    }
  }
...
for(pdf_file in pdf_files){
  ...
  # Extract text from all pages; returns NULL on error (e.g. encrypted PDF).
  pdf_text_raw <- tryCatch(pdftools::pdf_text(pdf_file), error = function(e) NULL)
  if(is.null(pdf_text_raw)) {
    cat("  WARNING: Could not read", doc_name, "— skipping.\n")
    next
  }
  ...
  clean_text <- paste(pdf_text_raw, collapse = " ") %>%
    ...
    # Normalize soft hyphens (Unicode U+00AD) that pdftools sometimes produces
    # from end-of-line hyphenation in PDFs, so hyphenated terms match correctly.
    str_replace_all("\u00AD", "-") %>%
    
    # Normalize common ligature artifacts from PDF font encoding
    # (e.g. "ﬁ" -> "fi", "ﬂ" -> "fl") that would break term matching.
    str_replace_all("\uFB01", "fi") %>%
    str_replace_all("\uFB02", "fl") %>%
    
    # Remove non-printable control characters that can appear in extracted PDFs.
    str_replace_all("[\\x00-\\x08\\x0b\\x0c\\x0e-\\x1f]", " ") %>%
    
    # Collapse multiple spaces into one and trim edges.
    str_squish() %>%
    
    # Lowercase everything so matching is case-insensitive at the text level
    # (regex also uses ignore_case = TRUE as a belt-and-suspenders measure).
    str_to_lower()
  ...
  writeLines(clean_text,
             file.path(folder_path, paste0(file_path_sans_ext(doc_name), "_cleaned.txt")))
  
  # Word count
  word_count <- str_split(clean_text, "\\s+")[[1]] %>% length()
  
  # Term frequencies
  freq_counts   <- count_terms_no_overlap(clean_text, terms)
...
if(length(results_list) == 0){
  stop("No documents were successfully processed. Check PDF files and paths.")
}
...
# Frequency table: one row per document, one column per term concept.
freq_df <- map_dfr(results_list, function(x){
  df <- as.data.frame(as.list(x$Frequencies), check.names = FALSE)
  cbind(Document = x$Document, WordCount = x$WordCount, df)
})

# Binary table: same structure but 0/1 presence flags; columns suffixed _binary.
binary_df <- map_dfr(results_list, function(x){
  df <- as.data.frame(as.list(x$Binary), check.names = FALSE)
  colnames(df) <- paste0(colnames(df), "_binary")
  cbind(Document = x$Document, df)
})
...
cat("\nTerm_Frequencies.csv and Term_Binary.csv written.\n")
...
cluster_results <- lapply(names(results_list), function(doc_name){
  ...
  # For each cluster, flag 1 if any member term is present in this document.
...
write.csv(binary_df_with_clusters,
          file.path(folder_path, "Term_Binary_with_Clusters.csv"),
          row.names = FALSE)

cat("Term_Binary_with_Clusters.csv written.\n")
cat("\nAnalysis complete with overlap-aware counting.\n")
